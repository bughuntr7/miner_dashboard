...
export default function IncentiveChart({ minerName, limit = 50 }: IncentiveChartProps) {
  ...
  useEffect(() => {
    const fetchIncentives = async () => {
      try {
        ...
        const chartData = incentives.map((inc: any) => {
            const timestamp = inc.timestamp || inc.datetime
  ...
  if (data.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold mb-4">Incentive History</h3>
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
      ...
      <ResponsiveContainer width="100%" height={400}>
        <LineChart data={data}>
          ...
          <YAxis 
            tick={{ fill: '#6b7280' }}
            label={{ value: 'Incentive', angle: -90, position: 'insideLeft' }}
          />
          <Tooltip 
